...
export default function ParentAttendance() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-gray-900 dark:text-surface-100">Children's Attendance</h1>
      {children && children.length > 0 ? children.map((child) => (
        <Card key={child.id}>
          <h3 className="font-semibold text-gray-900 dark:text-surface-100 mb-3">{child.first_name} {child.last_name}</h3>
          <p className="text-sm text-gray-500">Attendance data will appear here.</p>
        </Card>
